...
'''
python .\hollywoodhead.py -d C:\works -o C:\works
'''
agnostic_label_map = {
    1: 'head',
}
def read_a_xml(xml,boxes_wh):
    ...
    for obj in root.findall('object'):
        ...
        boxes_wh[fname][1].append((xmin,ymin,xmax-xmin,ymax-ymin,name_class))
...
def main(args):
    root = args.data_root
    path = join(root,'HollywoodHead\HollywoodHeads\Annotations')
    
    split_path = join(root,'HollywoodHead\HollywoodHeads\Splits')
    split_dict = {'train.txt':[],'val.txt':[],'test.txt':[]}
    ...
    for subset in split_dict.keys():
        ...
        with open(join(args.output_path,'HollywoodHead_{}.json'.format(subset.split('.')[0])), 'w') as json_fp:
